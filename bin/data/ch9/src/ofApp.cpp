...
void ofApp::setup() {
	...
	shieldMesh.load("ch9/shield.ply");
	calcTangents(shieldMesh);
	uvShader.load("ch9/mesh.vert", "ch9/blinnphong.frag");

	waterMesh.load("ch9/water.ply");
	calcTangents(waterMesh);
	waterShader.load("ch9/water.vert", "ch9/water.frag");

	dlight.direction = normalize(vec3(0.2, -1, -0.3));//顶部向下
	dlight.color = vec3(1, 1, 1);//白色
	dlight.intensity = 1;//强度

	diffImage.load("ch9/shield_diffuse.png");
	specImage.load("ch9/shield_spec.png");
	normalImage.load("ch9/shield_normal.png");
	waterNormalImage.load("ch9/water_nrm.png");
	//图片uv越界处理模式，重复
	...
	skyMesh.load("ch9/cube.ply");
	skyShader.load("ch9/cube.vert", "ch9/cube.frag");
	cubeMap.load("ch9/cube_front.jpg", "ch9/cube_back.jpg", "ch9/cube_right.jpg", "ch9/cube_left.jpg", "ch9/cube_top.jpg", "ch9/cube_bottom.jpg");
...
void ofApp::draw() {
	...
	uvShader.setUniform3f("ambientCol", glm::vec3(0.3, 0.3, 0.3));
